...
export default class Login extends Component {
  ...
  componentDidMount() {
    ...
    AjaxUtil.chkToken(true).then(success => {
      if(!success) return isBrowserHistory ? browserHistory.push('/pet/table') : hashHistory.push('/pet/table');
      this.setState({
        isChkToken: true
      });
    });
    ...
    if(this.state.isChkToken) {
      const login = (e) => {
        ...
        .then(({token, message, dom} = data) => {
          ...
          isBrowserHistory ? browserHistory.push('/pet/table') : hashHistory.push('/pet/table');
